...
pub async fn set(app: &str, pair: &str) {
    ...
    let (key, value) = match pair.split_once('=') {
        ...
        None => { eprintln!("Invalid format. Use KEY=value"); std::process::exit(1); }
    };

    match client.post::<serde_json::Value>(&format!("/apps/{app}/env"), &serde_json::json!({"key": key, "value": value})).await {
...
pub async fn list(app: &str) {
    ...
    match client.get::<serde_json::Value>(&format!("/apps/{app}/env")).await {
        Ok(resp) => {
            let vars = resp.get("data").and_then(|v| v.as_array()).cloned().unwrap_or_default();
            if vars.is_empty() {
