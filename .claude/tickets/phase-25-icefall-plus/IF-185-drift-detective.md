...
The agent continuously checksums running container configs, env vars, and network rules against the declared state in SQLite. Any out-of-band change (manual `docker exec`, someone SSH'd in and edited something) triggers an alert with a diff and one-click revert to the declared state.
...
- Use `docker inspect` to get the running container's config, compare against the app record in SQLite
...
- IF-004 (Docker Engine client — container inspect)
