...
Visualize the relationships between apps and databases as an interactive dependency graph. Shows which apps are linked to which databases, which services share environment variables, and which containers are on the same Docker network. Helps users understand their infrastructure at a glance.
...
- IF-073 (Docker Compose — service relationships)
