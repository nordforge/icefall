...
- [ ] Image available on worker (via transfer or remote build — see IF-132)
...
- [ ] Deploy record updated: pending → building → transferring → starting → health_check → routing → live → (or failed)
...
- The "transferring" status is new — add it to the DeployStatus enum in the database
