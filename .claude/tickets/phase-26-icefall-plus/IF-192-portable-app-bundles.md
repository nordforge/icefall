...
Export a fully self-contained app definition as a shareable `.icefall` file. Anyone with Icefall can import it and have the app configured in seconds. Includes container config, env var templates, build settings, resource limits, health checks, and domain patterns — everything except secrets. Runtime-agnostic: bundles work with both Docker and Podman.
...
- For container image apps: include `image`, `tag`, `port`, `env_template`
- For Compose apps: include the full compose YAML content inline
