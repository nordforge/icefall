# IF-170: Docker registry credentials & container registry management
...
Allow users to configure credentials for private Docker registries so they can deploy private images (IF-065) and optionally push built images to a registry. Supports Docker Hub, GitHub Container Registry (GHCR), GitLab Registry, and custom registries.
...
### Pull Integration
- [ ] When deploying a Docker image app (IF-065): if the image URL matches a configured registry, use those credentials for `docker pull`
- [ ] Registry selection dropdown on the Docker image deploy form
...
### Dashboard UI
- [ ] Settings page: "Docker Registries" section
