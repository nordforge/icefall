...
Allow users to configure commands that run inside the container after a successful deployment. Common use cases: database migrations, cache warming, asset compilation, seed data. Commands execute via `docker exec` in the newly deployed container.
...
- [ ] After container starts and health check passes, execute each command sequentially via `docker exec`
...
- IF-077 (Container terminal — uses the same Docker exec mechanism)
